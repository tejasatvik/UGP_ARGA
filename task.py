...
class Task:
    def __init__(self, filepath, allowed_abs=None):
        """
        filepath: path to the ARC json (e.g., dataset/training/00d62c1b.json)
        allowed_abs: optional list of abstraction labels to restrict the search to
                     e.g., ["ccg", "nbccg", "lrg"]
        """
        # Resolve which abstractions to use
        def _resolve_abstractions(allowed):
            # Image.abstractions may be a list or a dict in different versions of the code.
            # Accept either and normalize to a list of labels.
            if isinstance(Image.abstractions, dict):
                full = list(Image.abstractions.keys())
            elif isinstance(Image.abstractions, (list, tuple)):
                full = list(Image.abstractions)
            else:
                # Fallback: if abstraction_ops mapping exists, use its keys; otherwise coerce to list
                if hasattr(Image, "abstraction_ops") and isinstance(Image.abstraction_ops, dict):
                    full = list(Image.abstraction_ops.keys())
                else:
                    full = list(Image.abstractions)
            if allowed is None:
                return full
            allowed = [a.strip() for a in allowed]
            valid = [a for a in allowed if a in Image.abstractions]
            if not valid:
                raise ValueError(
                    f"No valid abstractions in {allowed}. "
                    f"Valid options: {full}"
                )
            return valid

        self.abstractions = _resolve_abstractions(allowed_abs)

        # list of all abstractions to consider during search (may be restricted by allowed_abs)
        self.all_possible_abstractions = list(self.abstractions)

        # mapping of abstraction -> allowed transformation ops (use ARCGraph definitions)
        self.all_possible_transformations = ARCGraph.transformation_ops
        ...
        self.time_limit = 600  # time limit for search
    ...
    def solve(self, shared_frontier=True, time_limit=600, do_constraint_acquisition=True, save_images=False):
        """
    ...
    def calculate_score(self, apply_call):
        ...
        # Use a fixed-size hash digest instead of converting a very long decimal string to int.
        # This avoids hitting Python 3.11's limit on digits for int(string) while still providing
        # a stable token for caching / deduplication (used in frontier_hash).
        digest = hashlib.sha256(token_string.encode("utf-8")).hexdigest()
        return score, digest
