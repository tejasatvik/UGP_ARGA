...
class ARCGraph:
    ...
    def rotate_node(self, node, rotation_dir: Rotation):
        ...
        # edge-case: if node has no pixels (size == 0), nothing to rotate
        # return early to avoid division by zero when computing the center
        if self.graph.nodes[node]["size"] == 0:
        # Skip rotation — invalid node structure
            return self
    ...
    def get_color(self, node):
        ...
        Safely handles None or invalid nodes.
        """
        if isinstance(node, list):
            return [
                self.graph.nodes[n]["color"]
                for n in node if n is not None and n in self.graph.nodes
            ]
        elif node is not None and node in self.graph.nodes:
            return self.graph.nodes[node]["color"]
        else:
            return None  # Safe fallback for invalid input
    ...
    def get_centroid(self, node):
        ...
        """
        # Protect against invalid input (None or node not present in graph)
        if node is None or node not in self.graph.nodes:
            return (0, 0)

        # Safely retrieve size and node pixels
        size = self.graph.nodes[node].get("size", 0)
        if size == 0:
            return (0, 0)

        nodes = self.graph.nodes[node].get("nodes", [])
        if not nodes:
            return (0, 0)

        center_y = (sum([n[0] for n in nodes]) + size // 2) // size
        center_x = (sum([n[1] for n in nodes]) + size // 2) // size
    ...
    def plot(self, ax=None, save_fig=False, file_name=None):
        ...
        if self.abstraction is None:
            ...
            # OLD (crashes when data["color"] is None)
            # color = [self.colors[data["color"]] for node, data in self.graph.nodes(data=True)]

            # NEW (safe)
            color = [
                self.colors[data["color"]]           # valid colour index
                if data.get("color") is not None and isinstance(data["color"], int)
                else "#000000"                       # fallback (black) for None / bad values
            ...
            # Build a safe node color list. Some nodes may store a list of colors
            # (when the graph is multicolor). For plotting choose a representative
            # color: the first valid integer color index in the list. If no valid
            # color is found, fall back to black.
            if self.abstraction == "mcccg":
                color = [self.colors[0] for node, data in self.graph.nodes(data=True)]
            else:
                # Quick debug: collect nodes with invalid/None color to help debugging
                invalid_color_nodes = []
                for node, data in self.graph.nodes(data=True):
                    c = data.get("color")
                    if c is None or (not isinstance(c, int) and not isinstance(c, (list, tuple))):
                        invalid_color_nodes.append((node, c))
                if len(invalid_color_nodes) > 0:
                    print(f"[ARCGraph.plot] warning: nodes with invalid color data: {invalid_color_nodes}")

                color = []
                for _, data in self.graph.nodes(data=True):
                    c = data.get("color")
                    # If color is an int, use it when valid
                    if isinstance(c, int) and 0 <= c < len(self.colors):
                        color.append(self.colors[c])
                        continue
                    # If color is a list/iterable, pick the first valid int index
                    if isinstance(c, (list, tuple)):
                        chosen = None
                        for item in c:
                            if isinstance(item, int) and 0 <= item < len(self.colors):
                                chosen = item
                                break
                        if chosen is not None:
                            color.append(self.colors[chosen])
                            continue
                    # Fallback: use black
                    color.append("#000000")

            # Safe size mapping with default of 1 to avoid crashes on missing data
            size = [300 * data.get("size", 1) for node, data in self.graph.nodes(data=True)]
            ...
            # Skip drawing edge labels to avoid matplotlib/nx unpacking bug
